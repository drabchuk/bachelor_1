 ...
 */
public class GdpRawDataAdditionFearuresSin implements RawData {
    ...
    public GdpRawDataAdditionFearuresSin(List<AnnualGDPelement> gdpList) {
    ...
    public TrainingData toTrainingData() {
        ...
        double[][] additionalFeatures = addExp(xNormalize);
        double[][] additionalFeaturesSin = addSin(additionalFeatures);
        return new TrainingData(addOnes(additionalFeaturesSin), yNormalize);
    }
    ...
    private static double[][] addExp(double[][] x) {
        double[][] res = new double[x.length][x[0].length + 1];
        for (int i = 0; i < x.length; i++) {
            res[i][x[0].length] = Math.exp(x[i][0]);
    ...
    private static double[][] addSin(double[][] x) {
        double[][] res = new double[x.length][x[0].length + 100];
        for (int i = 0; i < x.length; i++) {
            int k = 0;
            double fi = 0.0;
            for (int f = 0; f < 10; f++) {
                ...
                for (int a = 1; a <= 10; a++) {
                    res[i][k++] = Math.sin(a * (x[i][0] + fi));
                }
            }
            res[i][0] = Math.exp(x[i][0]);
            for (int j = 1; j <= x[0].length; j++) {
                res[i][j] = x[i][j - 1];
            }
        }
